...
class SegmentationDecoder(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        self.deconv_layers = nn.Sequential(
            nn.ConvTranspose2d(32, 64, kernel_size=4, stride=2, padding=1, output_padding=1), # 8*8 -> 16*16
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.ConvTranspose2d(64, 128, kernel_size=4, stride=2, padding=1, output_padding=1), # 16*16 -> 32*32
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1, output_padding=1), # 32*32 -> 64*64
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1, output_padding=1), # 64*64 -> 128*128
            nn.BatchNorm2d(32),
            nn.ReLU(),
            nn.ConvTranspose2d(32, num_classes, kernel_size=3, stride=2, padding=1, output_padding=1), # 128*128 -> 256*256
        )
        self._init_weights()
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)  # Ensure exact size
        return x
...
def assign_mask_to_patch(mask_data, x_patch, y_patch, patch_size, padding_value=0):
    ...
    padded_mask = np.pad(mask_data, ((pad_y, pad_y_end), (pad_x, pad_x_end)), mode='constant', constant_values=padding_value)
...
def construct_segmentation_labels(
    ...
    for case_idx, case in enumerate(cases):
        ...
        for i, (x_patch, y_patch) in enumerate(patch_coordinates):
            ...
            if is_train:
                segmentation_mask_patch = assign_mask_to_patch(segmentation_mask, x_patch, y_patch, patch_size)
            else:
                segmentation_mask_patch = None

            processed_data.append((patch_emb, segmentation_mask_patch, (x_patch, y_patch), f"{case}"))
...
def custom_collate(batch):
    ...
    if all(segmap is None for segmap in segmentation_masks):
        ...
        segmentation_masks = default_collate([segmap for segmap in segmentation_masks if segmap is not None]) # create a tensor from all the non-None segmentation masks in the batch.
...
def train_decoder(decoder, dataloader, num_epochs=200, lr=0.001):
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=0)  # targets are class labels (not one-hot)
    ...
    for epoch in range(num_epochs):
        ...
        for patch_emb, target_mask, _, _ in dataloader:
            ...
            target_mask = target_mask.long()  # Convert to LongTensor for CrossEntropyLoss
...
def inference(decoder, dataloader, patch_size=224, test_image_sizes=None):
    ...
    with torch.no_grad():
        ...
        for patch_emb, _, patch_coordinates_batch, case in dataloader: # patch_emb, segmentation_mask_patch, patch_coordinates, case
            patch_emb = patch_emb.to(device)

            pred_masks = decoder(patch_emb)
            pred_masks = torch.argmax(pred_masks, dim=1)  # gives a [batch_size, height, width] tensor with class labels

            patch_predictions.append(pred_masks.cpu().squeeze(0).numpy())  # Store predicted heatmap (convert to numpy)
            patch_coordinates.extend(patch_coordinates_batch)  # Store coordinates of the patch
            roi_identifiers.extend([case] * len(patch_coordinates_batch))  # Store the case identifier for each patch

    predicted_masks = {}
    for pred_masks, (x, y), case in zip(patch_predictions, patch_coordinates, roi_identifiers):
        ...
        if slice_height > 0 and slice_width > 0:
            ...
            predicted_masks[case][x:x + slice_width, y:y + slice_height] = pred_masks_resized
        else:
